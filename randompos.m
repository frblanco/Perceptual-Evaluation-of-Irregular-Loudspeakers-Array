...
for i=1:5:size(reference,2)*5;
    ...
    for j=1:channels
        ...
        arrayAzimuths = ((version1(j,2)-channel_zone+step):step:(version1(j,2)+channel_zone-step));
    ...
    switch channels
        ...
        case 12
           ...
           switch version2(j,3)
            ...
            version2(11:12,2) = tmp; % sorted in descending order, just assign (11) to Upper Left Surround Rear and 
    ...
    for j=1:1:size(version3,1)
        idx = randi([1,length(distance)]); %Random Index to select one of the possible distances from "distance"
        version3(j,4) = distance(  idx  ); %Overrite the reference value with the new one. 
        distance(idx) = []; %Delete the value used above so there is no same value for the distances
    end         
    ...
    switch channels
        ...
        case 5
            ...
            % Delta Ch 1-3 must at least the step size          
            % Delta Ch 2-3 must at least the step size
            [tmp] = chn3mapping(version4(1,2) , version4(2,2), version4(3,2), step);
